...
const Sidebar: React.FC = () => {
  ...
  const activePath = matchingNavItems.sort((a, b) => b.path.length - a.path.length)[0]?.path;
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed left-0 top-0 h-screen bg-slate-900 text-slate-400 flex flex-col z-50 overflow-hidden shadow-2xl transition-transform duration-300 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'
          } lg:translate-x-0`}
